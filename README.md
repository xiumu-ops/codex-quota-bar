...
右键额度条，选择 **外观配置 > 编辑个性配色**，可在统一风格的窗口中输入全部 19 项颜色并查看即时色块预览。颜色只接受严格的 `#RRGGBB` 格式；确认前会逐项校验，错误项会定位并以内联提示标出。确认后自动保存并启用个性外观。也可以通过“使用默认外观 / 使用个性外观”在 `Default` 与 `Custom` 间切换；`Default` 使用 `config-default.json` 的完整基线，但不会删除已经填写的个性值。

**高级配置：用户文件 / 默认文件** 用于手动维护字体或完整配置，保存后选择 **重新加载外观** 应用。普通配色修改不需要直接编辑 JSON。
